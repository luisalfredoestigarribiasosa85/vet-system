      ...
      {(() => {
        ...
        return filtered.length === 0 ? (
          ...
          <>
                      ...
                      <tr key={it.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-sm text-gray-700">
                          <div className="flex gap-2">
                            <Button variant="secondary" onClick={() => openModal(it)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                              <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                            </svg>
                            </Button>
                            <Button variant="danger" onClick={() => handleDelete(it.id)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
            ...
            <div style={{ display: isMobile ? 'block' : 'none' }} className="space-y-4">
              {filtered.map((it) => (
                <div key={it.id} className="bg-white rounded-xl shadow p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-800 text-lg">{it.name}</h3>
                      {it.category && (
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className={`font-semibold ${it.quantity <= it.minStock ? 'text-red-600' : 'text-gray-800'}`}>
                        {it.quantity}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-500">Mínimo</p>
                      <p className="font-semibold text-gray-800">{it.minStock}</p>
                    </div>
                    {it.supplier && (
                      <div className="col-span-2">
                        <p className="text-gray-500">Proveedor</p>
                        <p className="font-semibold text-gray-800">{it.supplier}</p>
                      </div>
                    )}
                    {it.expiryDate && (
                      <div className="col-span-2">
                        <p className="text-gray-500">Vencimiento</p>
                        <p className="font-semibold text-gray-800">{it.expiryDate}</p>
                      </div>
                    )}
                  </div>
                  {it.quantity <= it.minStock && (
                    <div className="mt-3 px-3 py-2 bg-red-50 border border-red-200 rounded-lg">
                      <p className="text-xs text-red-800 font-medium">⚠️ Stock bajo</p>
                    </div>
                  )}
                </div>
              ))}
